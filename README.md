...
1. 安装 MikTeX ([link1](https://miktex.org/), [link2](https://blog.csdn.net/DrGuCoding/article/details/123523407)), 比 TeX-Live 更轻量化
2. 安装 LaTeX Workshop 插件，设置 ctrl+s 自动保存编译
3. 编译使用`latexmk`或者`pdflatex->biber->pdflatex->pdflatex`, 设置`latexmk`为默认 Recipe
4. 常用跳转快捷键：
    - from PDF to LaTeX: ctrl + double-click
    - LaTeX to PDF: select, alt + v
    - clean auxuliary files: ctrl + alt + c
5. 在MikTeX console中安装`cn-super`和`cn-unicode`包，不需要`\usepackage`即可改善字体（消除锯齿感）

示范 settings.json 配置文件：
...
```json
{
    ...
    "latex-workshop.latex.recipes": [
        ...
        {
            "name": "(for disser) pdflatex/biber/pdflatex/pdflatex", // 注意使用biber而不是bibtex
    ...
    "latex-workshop.latex.recipe.default": "latexmk", // 默认编译Recipe
...
_Russian-Phd-LaTeX-Dissertation-Template_ благодарность за помощь в
